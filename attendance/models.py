...
class WebAuthnCredential(models.Model):
    """
    Stores the Public Key and Credential ID generated by the student's 
    biometric hardware (Fingerprint/FaceID).
    """
    ...
    # The unique ID for the hardware credential
    credential_id = models.CharField(max_length=255, unique=True)
    # The public key used to verify future signatures
    public_key = models.TextField()
    # Prevents "cloned" authenticators by tracking the signature count
    sign_count = models.IntegerField(default=0)
    # Friendly name for the device (e.g., "Samsung S24", "iPhone 15")
...
class WebAuthnChallenge(models.Model):
    """
    Temporary storage for the cryptographic challenge sent to the device.
    Must be deleted immediately after a successful verification.
    """
