...
class WebAuthnCredential(models.Model):
    """
    Stores the public key for a user's registered biometric device.
    One user can have multiple credentials (e.g. phone + laptop).
    """
    ...
    credential_id = models.TextField(unique=True)  # identifies the key pair on the device
    public_key    = models.TextField()             # used to verify signatures on login
    aaguid        = models.TextField(default='')   # authenticator device identifier — used to prevent cross-account registration
    sign_count    = models.IntegerField(default=0) # increments each login — detects cloned credentials
    device_name   = models.CharField(max_length=100, default='My Device')
...
class WebAuthnChallenge(models.Model):
    """
    Temporarily stores the random challenge sent to the device.
